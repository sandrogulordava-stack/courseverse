...
export function publicUser(user) {
  if (!user) return null;
  return { id:user.id, name:user.name, email:user.email, avatar:user.avatar, role:user.role, headline:user.headline, bio:user.bio };
}
...
export function registerAuthRoutes(app) {
  app.post('/api/auth/register', (req,res) => {
    ...
    const user = { id:uuid(), name, email:email.toLowerCase(), password_hash:bcrypt.hashSync(password,10), role, avatar:`https://api.dicebear.com/9.x/initials/svg?seed=${encodeURIComponent(name)}` };
